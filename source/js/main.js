...
const GOOGLE_CLIENT_ID =
  "736230719726-u4c6ik0sscous4930ruld7i0h20dflb4.apps.googleusercontent.com";
...
module.exports = function (app, ecoData, bcrypt, saltRounds) {
    ...
    new GoogleStrategy(
      ...
      async (req, accessToken, refreshToken, profile, done) => {
        try {
          ...
          const user = await (await MQL.getMongoDBInstance())
          ...
          if (!user) {
            ...
            await (await MQL.getMongoDBInstance())
              .collection("User_Credentials")
              .updateOne({}, { $push: { credentials: newUser } });
          } else {
  ...
  passport.deserializeUser(async (username, done) => {
    try {
      ...
      const user = await db
        .collection("User_Credentials")
        .findOne({ "credentials._id": username.displayName });
      if (user) {
  ...
  app.post("/login", async (req, res) => {
    ...
    try {
      const user = await (await MQL.getMongoDBInstance())
        ...
        .findOne(
          { "credentials.username": username },
          { projection: { credentials: { $elemMatch: { username } } } }
        );
        ...
        if (password === userData.password) {
          ...
          req.session.save((err) => {
            if (err) {
              ...
              console.log(
                "Loaded preferences into session:",
              ...
              console.log(
                "Loaded saved data into session:",
                req.session.usersaveddata
              );
  ...
  app.post("/register", async (req, res) => {
    ...
    try {
      ...
      const existingUser = await (await MQL.getMongoDBInstance())
        .collection("User_Credentials")
        .findOne({ "credentials._id": username });
      if (existingUser) {
        res.render("register.ejs", {
  ...
  app.get("/ecoTravelRoutes", isAuthenticated, async (req, res) => {
    try {
      ...
      let extendedEcoData = [];
      res.render("mapboxantpath.ejs", {
        extendedEcoData: extendedEcoData,
        username: req.session.username || null,
      });
    } catch (error) {
  ...
  app.post("/calculateRoute", async (req, res) => {
    try {
      ...
      const extendedEcoData = await ecolutionTravelRoutes.getRouteWaypoints(
        origin,
        destination,
        travelMode,
        EVMode
  ...
  app.post("/commitjourneycarbon", async (req, res) => {
    try {
      ...
      if (
        (req.session.cachedMTRoute != null ||
          req.session.cachedMTRoute != undefined) &&
        req.session.cachedMTRoute != 0
      ) {
        // Add the new journey carbon to the users standing amount
        const succesfullyUpdated = await MQL.AddToTotalEmission(
          req.session.username,
          req.session.cachedMTRoute
        );
        const succesfullyUpdated2 = await MQL.AddToDailyEmissions(
          req.session.username,
          req.session.cachedMTRoute
        );
        ...
        console.error(
          "Error trying to commit new journey carbon, make sure a route has been chosen"
        );
  ...
  app.post("/updateUserPreferences", async (req, res) => {
    try {
      const {
        avgAcceptableWalkingDist_mile,
        GBPostalPrefix,
        GBPostalSuffix,
        region,
        publicProfile,
      } = req.body;
      const userPreferences = await MQL.updateUserPreferences(
        req.session.username,
        avgAcceptableWalkingDist_mile,
        GBPostalPrefix,
        GBPostalSuffix,
      ...
      if (userPreferences) {
        req.session.userpreferences = await MQL.getUserPreferences(
          req.session.username
        );
        req.session.save((err) => {
          if (err) {
  ...
  app.get("/chart-data", async (req, res) => {
    try {
      const allDailyEmissions = await MQL.getUserAllDailyEmissions(
        req.session.username
      );

      // Extract the emissions values for the recent 7 days
      const recentData = allDailyEmissions
        .slice(-7)
        .map((entry) => parseFloat(entry[1]) || 0);
  ...
  app.get("/current-power-consumption", async (req, res) => {
    try {
      ...
      const consumptionData =
        await cbreakdown.getCurrentPowerConsumptionBreakdown(
          zoneID,
          postcodePrefix
        );
      const regionName = zoneID;
      ...
      console.error(
        "Failed to get current power consumption breakdown:",
        error
      );
  ...
  app.get("/get-recycling-centres", async (req, res) => {
    try {
      ...
      if (!postcodePrefix || !postcodeSuffix) {
        return res
          .status(400)
          .send("User postcode prefix and suffix are required.");
      }

      const centresData = await recycle.getNearestRecyclingCentresByPostCode(
        postcodePrefix,
        postcodeSuffix,
        radius,
        count
